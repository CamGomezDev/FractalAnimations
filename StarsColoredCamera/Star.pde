public class Star {
    ...
    Star(PVector center, float length, int numVertex, float thickness, int colIndex, float weightStroke, float swmin, float swmax) {
        this.length = length;
        this.center = center;
    ...
    ArrayList<Star> getChildrenStars() {
        ...
        for (int i = 0; i < numVertex; ++i) {
            childrenStars.add(new Star(
                ...
                weightStroke*reductionFactor, swmin*reductionFactor,
                swmax*reductionFactor
            ));
        }
